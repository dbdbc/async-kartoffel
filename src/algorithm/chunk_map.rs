...
impl Add<ChunkIndex> for InChunkIndex {
    type Output = Position;

    fn add(self, rhs: ChunkIndex) -> Self::Output {
        rhs.to_pos() + self.to_dist()
    }
}

pub struct IterInChunk {
    in_chunk_index: Option<InChunkIndex>,
}
impl IterInChunk {
    pub fn new() -> Self {
        Self {
            in_chunk_index: Some(InChunkIndex::first()),
        }
    }
}
impl Iterator for IterInChunk {
    type Item = InChunkIndex;
...
impl ChunkIndex {
    ...
    pub fn dist_to(&self, pos: Position) -> Distance<Global> {
        let dist_anchor = pos - self.to_pos();
...
impl<const N: usize, T, C: Chunk<T>> ChunkMap<N, T, C> {
    ...
    pub fn to_chunk_pos(pos: Position) -> (ChunkIndex, InChunkIndex) {
    ...
    pub fn get_mut_chunk_or_new(&mut self, index: ChunkIndex) -> Result<&mut C, Error> {
...
impl ChunkBool {
    fn bit(index: InChunkIndex) -> u64 {
        1u64 << index.index64()
    }
    pub fn get(self, index: InChunkIndex) -> bool {
        (self.data & Self::bit(index)) > 0
    }
    pub fn set(&mut self, index: InChunkIndex, val: bool) {
        if val {
